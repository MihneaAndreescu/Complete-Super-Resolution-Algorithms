...
class preUpsampling8(nn.Module):
    modelName = "preUpsampling8.pth"

    def __init__(self):
        super(preUpsampling8, self).__init__()
        ...
        self.conv4 = nn.Conv2d(8, 4, kernel_size = 7, padding = 3, padding_mode = 'replicate')
        self.conv5 = nn.Conv2d(4, 2, kernel_size = 7, padding = 3, padding_mode = 'replicate')
        self.conv6 = nn.Conv2d(2, 1, kernel_size = 7, padding = 3, padding_mode = 'replicate')
    def forward(self, x):
...
def preUpsampling8train():
    model = preUpsampling8().cuda()
    try:
    ...
    for i in range(1, 101):
        ...
        lr = 1e-4
        print("88888888888888888888888888888888 1e-4")
...
def preUpsampling8evaluate(img):
    model = preUpsampling8().cuda()
    try:
        model.load_state_dict(torch.load(model.modelName))
    except:
